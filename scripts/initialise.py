...
def dataimport(filename):
    ...
    #creates a list of all tine steps between start and end date using a calendar
...
def annualvol(startyear, dateList):
    ...
    for year in simyears: #range(1990, dateList[analysis_p].year+1):
...
def genstorm(volrange, freq, length, width):
    ...
    seasrain = np.array([188.8, 265.9, 130.6, 81.4]) #spring, summer, autumn, winter
