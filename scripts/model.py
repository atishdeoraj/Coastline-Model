...
#   Generate coastline coordinates
...
#   Initialise grid for tracking Gaussian nourishment inputs

gaussy = np.array([float(0) for i in coasty])
gaussx = coastx[:]

#   Cross-shore profile parameters

A = xshoreprofile(d50arr)                                       # shape factor
d50 = np.mean(d50arr)
...
input_hours = 0     #time over which storm is spread

iteration_count = int(analysis_p/delta_t)       #number of time steps in simulation
...
for dataset in data_dict:
    ...
    if dataset == stormdates[storm_index]:  #   If a storm is occurring
    ...
    if np.any(gaussy != 0): #   Checks if a Gaussian input exists
    ...
    #-------------- Adjust transport rates if necessary, where regions of no sediment exist  -------------#
